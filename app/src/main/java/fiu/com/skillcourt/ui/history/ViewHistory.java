...
/*
  ...
  */
            ...
            public void onDataChange(DataSnapshot snapshot) {
                ...
                for (DataSnapshot postSnapshot : snapshot.getChildren()) {
                   ...
                   Object date = map.get((Object)"date");
                    ...
                    Object green = map.get((Object)"totalHits");
                    totalList.add((Long) green);

                    Object times = map.get((Object)"gameTimeTotal");
                    totalList.add((Long) green);


                    //System.out.println("SCORE:" + scoresList + "  DATES:" + datesList + "Total Hits" + totalList + "Green Hits" + greenList + "Red Hits" + (totalList.get(0) - greenList.get(0)));
                }

                createLineGraph(scoresList, datesList);
                createHitIcons(greenList, totalList);
                createBarGraph(greenList, totalList, datesList);
                //createHorizontalBarGraph(greenList, totalList, datesList);
            }
    ...
    protected void createLineGraph(ArrayList<Long> scoresList, ArrayList<Long> datesList)
    {
        ...
        for (int i=0; i < scoresList.size(); i++) {
            float f1 = (float) i;
            vals.add(new Entry(f1, Float.parseFloat(scoresList.get(i).toString())));
        }
    ...
    protected void createBarGraph(ArrayList<Long> redList, ArrayList<Long> greenList, ArrayList<Long> datesList) {
        ...
        for (int i=0; i < redList.size(); i++) {
            float f1 = (float) i;
            vals.add(new BarEntry(f1, Float.parseFloat(redList.get(i).toString())));
        ...
        String[] values = new String[datesList.size()];
        for (int j=0; j < datesList.size(); j++) {
            values[j] = datesList.get(j).toString().substring(0,1) + "/" + datesList.get(j).toString().substring(2,3) + "/" + datesList.get(j).toString().substring(4,5);
        }
    ...
    protected void createHorizontalBarGraph(ArrayList<Long> redList, ArrayList<Long> greenList, ArrayList<Long> datesList) {
        ...
        for (int i=0; i < redList.size(); i++) {
            float f1 = (float) i;
            vals.add(new BarEntry(f1, Float.parseFloat(greenList.get(i).toString())));
        }

        BarDataSet dataSet = new BarDataSet(vals, "Length of Game");
        dataSet.setColor(Color.rgb(155, 0, 0));
